...
public:
    ...
    // THEN a new scan with serialNumber has been added in the right place to the list
    // ELSE  timesRecycled has been increased by one
    
    Scan* getScanByNr(int position);
    // pre: -
    // post: IF 0 <= position < nr of scans in the list // THEN return a pointer to the scan at that position // ELSE return NULL
    ...
    // THEN scan has been removed from the list and true has been returned // ELSE false has been returned
